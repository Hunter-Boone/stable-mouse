...
public:
    ...
    bool start(const QString &device) override {
        ...
        process.start("pkexec", {helper, device.section(" | ", 0, 0), QString::number(config.strength), QString::number(config.speed), config.centerTracking ? "1" : "0", config.alwaysCenter ? "1" : "0", QString::number(config.centerWindow)});
...
private:
    QByteArray command() const { return "CONFIG " + QByteArray::number(config.strength) + " " + QByteArray::number(config.speed) + (config.centerTracking ? " 1 " : " 0 ") + (config.alwaysCenter ? "1 " : "0 ") + QByteArray::number(config.centerWindow) + "\n"; }
    FilterConfig config;
